 ...
 */
@:access(states.PsychFreeplayState)
class MusicPlayer extends FlxGroup 
{
	public var instance:PsychFreeplayState;
	...
	public function new(instance:PsychFreeplayState)
	{
		super();
		...
		for (i in 0...2)
		{
			...
			var text = new FunkinText(0, 0, 0, '', 32, false, 'center');
			text.text = '^';
			if (i == 1)
				text.flipY = true;
			text.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		var songName:String = instance.songs[PsychFreeplayState.curSelected].songName;
		...
		//if(FlxG.keys.justPressed.K) trace('Time: ${PsychFreeplayState.vocals.time}, Playing: ${PsychFreeplayState.vocals.playing}');
		...
		if (playing)
		{
			if(PsychFreeplayState.vocals != null)
				PsychFreeplayState.vocals.volume = (PsychFreeplayState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
			if(PsychFreeplayState.opponentVocals != null)
				PsychFreeplayState.opponentVocals.volume = (PsychFreeplayState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;

			if((PsychFreeplayState.vocals != null && PsychFreeplayState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - PsychFreeplayState.vocals.time) >= 25) ||
			(PsychFreeplayState.opponentVocals != null && PsychFreeplayState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - PsychFreeplayState.opponentVocals.time) >= 25))
			{
				pauseOrResume();
	...
	{
		if (PsychFreeplayState.vocals != null && PsychFreeplayState.vocals.length > time)
			PsychFreeplayState.vocals.time = time;
		if (PsychFreeplayState.opponentVocals != null && PsychFreeplayState.opponentVocals.length > time)
			PsychFreeplayState.opponentVocals.time = time;
	}
	...
	public function pauseOrResume(resume:Bool = false) 
	{
		if (resume)
		{
			...
			if (PsychFreeplayState.vocals != null && PsychFreeplayState.vocals.length > FlxG.sound.music.time && !PsychFreeplayState.vocals.playing)
				PsychFreeplayState.vocals.resume();
			if (PsychFreeplayState.opponentVocals != null && PsychFreeplayState.opponentVocals.length > FlxG.sound.music.time && !PsychFreeplayState.opponentVocals.playing)
				PsychFreeplayState.opponentVocals.resume();
		}
		else 
		{
			...
			if (PsychFreeplayState.vocals != null)
				PsychFreeplayState.vocals.pause();
			if (PsychFreeplayState.opponentVocals != null)
				PsychFreeplayState.opponentVocals.pause();
		}
	}
	...
	{
		var length:Int = instance.songs[PsychFreeplayState.curSelected].songName.length;
	...
	function setPlaybackRate() 
	{
		...
		if (PsychFreeplayState.vocals != null)
			PsychFreeplayState.vocals.pitch = playbackRate;
		if (PsychFreeplayState.opponentVocals != null)
			PsychFreeplayState.opponentVocals.pitch = playbackRate;
	}
